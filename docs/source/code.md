...
```{py:function} Lookup(IP, lang)
Retrieve geolocation information for an IP address.

:param string IP: (Required) The IP address (IPv4 or IPv6).
:param string lang: (Optional) Translation information(ISO639-1). The translation is only applicable for continent, country, region and city name.
:::{note}
